...
$hdpath = "$CFG->dirroot/local/helpdesk";
...
function local_helpdesk_render_navbar_output(\renderer_base $renderer) {
    global $CFG;
// <div class="count-container hidden" data-region="count-container" aria-label="">0</div>
// <i class="fab fa-battle-net" title="מערכת קריאות" aria-label="מערכת קריאות"></i>
$aa = '<div class="popover-region collapsed" data-region="popover-region-messages">
    <a id="message-drawer-toggle-5ee8c27c6ba775ee8c27c493a512" class="nav-link d-inline-block popover-region-toggle position-relative" 
    href="https://helpdesk.openapp.co.il/dev/local/helpdesk/index.php" role="button">
    <i class="icon fa fa-sitemap" title="מערכת קריאות" aria-label="מערכת קריאות"></i>
    </a>
...
/**
 * Gets a speicificly formatted date string for the helpdesk local.
 *
...
function helpdesk_is_capable($capability=null, $require=false, $user=null) {
    ...
    if (empty($capability)) {
        ...
        if ($require !== false) {
            echo $OUTPUT->notification(get_string('warning_getandrequire', 'local_helpdesk'));
        }
...
function helpdesk_get_user($userid) {
    ...
    if (empty($userid)) {
        print_error('missingidparam', 'local_helpdesk');
    }
    static $users = array();
    ...
    $sql = "
        ...
        FROM {user} AS u
        LEFT JOIN {local_helpdesk_hd_user} AS hu ON hu.userid = u.id
        WHERE u.id = $userid
    ";
    $user = $DB->get_record_sql($sql, null, $strictness = IGNORE_MULTIPLE);
    if (!$user) {
        return false;
    }
...
function helpdesk_ensure_hd_user($userid) {
    ...
    $hd_user = $DB->get_record('local_helpdesk_hd_user', array('userid' => $userid));
    if (!$hd_user) {
...
function helpdesk_create_hd_user($userid) {
    ...
    return $DB->insert_record('local_helpdesk_hd_user', $hd_user);
}
...
function helpdesk_get_hd_user($hd_userid) {
    ...
    $sql = "
        ...
        FROM {local_helpdesk_hd_user} AS hu
...
function helpdesk_user_link($user) {
    ...
    if (isset($user->userid)) {
        ...
        $url = new moodle_url("$CFG->wwwroot/local/helpdesk/userlist.php");
        $url->param('hd_userid', $user->hd_userid);
        $url->param('function', HELPDESK_USERLIST_MANAGE_EXTERNAL);
        $type = get_string('nonmoodleuser', 'local_helpdesk');
    }
...
function helpdesk_authenticate_token($ticketid, $token) {
    ...
    if (empty($CFG->local_helpdesk_external_user_tokens)) {
        print_error('invalidtoken', 'local_helpdesk');
    }
    if (!$watcher = $DB->get_record('local_helpdesk_watcher', array('token' => $token, 'ticketid' => $ticketid))) {
        print_error('invalidtoken', 'local_helpdesk');
    }

    if (!isset($CFG->local_helpdesk_token_exp)) {
        $token_exp = HELPDESK_DEFAULT_TOKEN_EXP;
    } else {
        $token_exp = $CFG->local_helpdesk_token_exp;
    }
...
function helpdesk_get_session_var($varname) {
    global $SESSION;
    if (isset($SESSION->local_helpdesk)) {
        return isset($SESSION->local_helpdesk->$varname) ?
	       $SESSION->local_helpdesk->$varname : false;
    }
    $SESSION->local_helpdesk = new stdClass;
    return null;
}
...
function helpdesk_set_session_var($varname, $value) {
    ...
    $SESSION->local_helpdesk->$varname = $value;
}
...
function helpdesk_simple_helpbutton($title, $name, $return=true) {
    global $OUTPUT;
    $result = $OUTPUT->help_icon($name, 'local_helpdesk', false);
    return $result;
...
function helpdesk_print_header($nav, $title=null) {
    ...
    $meta = "<meta http-equiv=\"x-ua-compatible\" content=\"IE=8\" />\n
	     <link rel=\"stylesheet\" type=\"text/css\" href=\"$CFG->wwwroot/local/helpdesk/styles.css\" />\n";
    ...
    if (!empty($USER->helpdesk_external)) {
        ...
        echo $OUTPUT->header();
        echo "<div class='external'>" . get_string('welcome', 'local_helpdesk') . fullname_nowarnings($USER) . "</div>";
        echo $OUTPUT->heading($DB->get_record('course', array('id' => SITEID))->fullname . ' ' . get_string('helpdesk', 'local_helpdesk'), 1);
        return;
    }

    $helpdeskstr = get_string('helpdesk', 'local_helpdesk');
    if (empty($title)) {
        $title = $helpdeskstr;
    }
 ...
 */
function local_helpdesk_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    global $DB, $USER;
    ...
    // Note: if ever the local_helpdesk_email_htmlcontent admin setting comes to support files,
    ...
    if (!($item = $DB->get_record('local_helpdesk_ticket' . $tablesuffix, array('id'=>$itemid)))) {
        return false;
    }
    ...
    if (!is_siteadmin()) {
        ...
        switch ($filearea) {
            case 'ticketnotes':
            case 'tickettag':
                $creatorid = $DB->get_field('local_helpdesk_ticket', 'userid', array('id' => $item->ticketid));
                break;
            case 'ticketdetail':
                $creatorid = $itemid;
                break;
    ...
    $fullpath = "/$context->id/local_helpdesk/$filearea/$itemid/$relativepath";
