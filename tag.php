...
$PAGE->set_url('/local/helpdesk/tag.php');
...
// Build Navigation
$ticketurl = new moodle_url("$CFG->wwwroot/local/helpdesk/view.php");
$nav = array();
$nav[] = array ('name' => get_string('helpdesk', 'local_helpdesk'),
                'link' => $ticketurl->out());
...
$nav[] = array ('name' => get_string('tags', 'local_helpdesk'));

$title = get_string('helpdesktagticket', 'local_helpdesk');
...
if (is_numeric($remove)) {
    if (!$ticket->remove_tag($remove)) {
        print_error('unabletoremovetag', 'local_helpdesk');
    }
    $url = new moodle_url("$CFG->wwwroot/local/helpdesk/view.php");
    $url->param('id', $tid);
    $url = $url->out();
    redirect($url, get_string('tagremoved', 'local_helpdesk'));
    ...
    redirect($url->out(), get_string('tagadded', 'local_helpdesk'));
}
