...
$qurl = new moodle_url("$CFG->wwwroot/local/helpdesk/search.php");
$viewurl = new moodle_url("$CFG->wwwroot/local/helpdesk/view.php");
...
$nav = array (
    ...
    array (
        'name' => get_string('assignments', 'local_helpdesk')
    )
);

$title = get_string('helpdeskassignuser', 'local_helpdesk');
helpdesk_is_capable(HELPDESK_CAP_ANSWER, true);

$hd = helpdesk::get_helpdesk();
$ticket = $hd->get_ticket($tid);
$returnurl = new moodle_url("$CFG->wwwroot/local/helpdesk/view.php");
...
if (!empty($remove)) {
    ...
    if(!$ticket->remove_assignment($uid)) {
        print_error('cannotremoveassignment', 'local_helpdesk');
    }
    $str_unassigned = get_string('hasbeenunassigned', 'local_helpdesk');
...
if (!empty($uid)) {
    ...
    if (!empty($assigned)) {
        foreach($assigned as $element) {
            ...
            $text = "<a href=\"$userurl\">" . fullname_nowarnings(helpdesk_get_user($element->userid)) .
                    "</a> " . get_string('isalreadyassigned', 'local_helpdesk');
            redirect($returnurl, $text);
    ...
    if(!helpdesk_is_capable(HELPDESK_CAP_ANSWER, false, $uid)) {
        print_error('usernotananswerer', 'local_helpdesk');
    }
    // We want to add the assignment here.
    if(!$ticket->add_assignment($uid)) {
        print_error('cannotaddassignment', 'local_helpdesk');
    }
    redirect($returnurl, get_string('assignmentadded', 'local_helpdesk'));
}
...
foreach($allassignables as $user) {
    ...
    $assignstr = get_string('assignthisuser', 'local_helpdesk');
    $table->data[] = array(
