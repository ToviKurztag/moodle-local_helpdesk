...
function addwatchertable($version, $dbman) {
    global $DB;
        ...
        // Define table local_helpdesk_hd_user to be created
        $table = new xmldb_table('local_helpdesk_hd_user');

        // Adding fields to table local_helpdesk_hd_user
        ...
        // Adding keys to table local_helpdesk_hd_user
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        // Adding indexes to table local_helpdesk_hd_user
        $table->add_index('local_helpdesk_hd_user_u_ix', XMLDB_INDEX_NOTUNIQUE, array('userid'));
        $table->add_index('local_helpdesk_hd_user_e_ix', XMLDB_INDEX_NOTUNIQUE, array('email'));
        $table->add_index('local_helpdesk_hd_user_p_ix', XMLDB_INDEX_NOTUNIQUE, array('phone'));

        $dbman->create_table($table);

        // Define table local_helpdesk_watcher to be created
        $table = new xmldb_table('local_helpdesk_watcher');

        // Adding fields to table local_helpdesk_watcher
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
        ...
        // Adding keys to table local_helpdesk_watcher
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        // Adding indexes to table local_helpdesk_watcher
        $table->add_index('local_helpdesk_watcher_t_ix', XMLDB_INDEX_NOTUNIQUE, array('ticketid'));
        $table->add_index('local_helpdesk_watcher_h_ix', XMLDB_INDEX_NOTUNIQUE, array('hd_userid'));
        $table->add_index('local_helpdesk_watcher_th_ux', XMLDB_INDEX_UNIQUE, array('ticketid', 'hd_userid'));
        ...
        $users = $DB->get_records_sql("
            ...
            UNION
            SELECT DISTINCT(userid)
            FROM {local_helpdesk_ticket_assign}
        ");
        foreach ($users as $record) {
            $hd_userid = $DB->insert_record('local_helpdesk_hd_user', $record);
        ...
        // Define field hd_userid to be added to local_helpdesk_ticket
        ...
        /// Define field createdby to be added to local_helpdesk_ticket
        $table = new xmldb_table('local_helpdesk_ticket');
        ...
        $watchers = array();
        $tickets = $DB->get_records('local_helpdesk_ticket');
        if (!empty($tickets)) {
            foreach ($tickets as $ticket) {
                # set createdby, hd_userid
                $ticket->createdby = $ticket->hd_userid = $user2hd_user[$ticket->userid];
                $DB->update_record('local_helpdesk_ticket', $ticket);
                ...
                $DB->insert_record('local_helpdesk_watcher', $watcher);
        ...
        $ticket_assigns = $DB->get_records('local_helpdesk_ticket_assign');
        ...
        // Changing nullability of field hd_userid on table local_helpdesk_ticket to not null
        $table = new xmldb_table('local_helpdesk_ticket');
        $field = new xmldb_field('hd_userid');
        ...
        /// Changing nullability of field createdby on table local_helpdesk_ticket to not null
        $table = new xmldb_table('local_helpdesk_ticket');
        $field = new xmldb_field('createdby');
        ...
        // Define index local_helpdesk_ticket_h_ix (not unique) to be added to local_helpdesk_ticket
        $table = new xmldb_table('local_helpdesk_ticket');
        $index = new xmldb_index('local_helpdesk_ticket_h_ix');
        $index->set_attributes(XMLDB_INDEX_NOTUNIQUE, array('hd_userid'));

        // Conditionally launch add index local_helpdesk_ticket_h_ix
        ...
        // Define field hd_userid to be added to local_helpdesk_ticket_update
        $table = new xmldb_table('local_helpdesk_ticket_update');
        $field = new xmldb_field('hd_userid');
        ...
        $ticket_updates = $DB->get_records('local_helpdesk_ticket_update');
        if (!empty($ticket_updates)) {
            foreach ($ticket_updates as $update) {
                $update->hd_userid = $user2hd_user[$update->userid];
                $DB->update_record('local_helpdesk_ticket_update', $update);
            }
        }
        ...
        // Define index idx_hd_t_userid (not unique) to be dropped form local_helpdesk_ticket
        $table = new xmldb_table('local_helpdesk_ticket');
        ...
        // Define field userid to be dropped from local_helpdesk_ticket
        $table = new xmldb_table('local_helpdesk_ticket');
        ...
        // Define field userid to be dropped from local_helpdesk_ticket_update
        $table = new xmldb_table('local_helpdesk_ticket_update');
        $field = new xmldb_field('userid');
        ...
        // Define index local_helpdesk_ticket_update_t_ix (not unique) to be added to local_helpdesk_ticket_update
        $table = new xmldb_table('local_helpdesk_ticket_update');
        $index = new xmldb_index('local_helpdesk_ticket_update_t_ix');
         ...
         * Drop helpdesk table
         */
        // Define table local_helpdesk to be dropped
         ...
         * Drop helpdesk_rule
         */
        // Define table local_helpdesk_rule to be dropped
        $table = new xmldb_table('local_helpdesk_rule');
         ...
         */
        // Define table local_helpdesk_rule_email to be dropped
        $table = new xmldb_table('local_helpdesk_rule_email');
         ...
         */
        // Define table local_helpdesk_ticket_group to be dropped
        $table = new xmldb_table('local_helpdesk_ticket_group');
        ...
        $table = new xmldb_table('local_helpdesk_ticket');
        $index = new xmldb_index('local_helpdesk_ticket_f_ix');
        $index->set_attributes(XMLDB_INDEX_NOTUNIQUE, array('firstcontact'));
        $dbman->add_index($table, $index);
        ...
        $table = new xmldb_table('local_helpdesk_ticket');
        $index = new xmldb_index('local_helpdesk_ticket_st_ix');
        $index->set_attributes(XMLDB_INDEX_NOTUNIQUE, array('status'));
        $dbman->add_index($table, $index);


        // on helpdesk_ticket_tag.ticketid
        $table = new xmldb_table('local_helpdesk_ticket_tag');
        $index = new xmldb_index('idx_hd_tt_ticketid');
        $index->set_attributes(XMLDB_INDEX_NOTUNIQUE, array('ticketid'));
        $dbman->drop_index($table, $index);

        $table = new xmldb_table('local_helpdesk_ticket_tag');
        $index = new xmldb_index('local_helpdesk_ticket_tag_t_ix');
        $index->set_attributes(XMLDB_INDEX_NOTUNIQUE, array('ticketid'));
        $dbman->add_index($table, $index);


        // on local_helpdesk_ticket_assign.userid
        $table = new xmldb_table('local_helpdesk_ticket_assign');
        $index = new xmldb_index('idx_hd_ta_userid');
        $index->set_attributes(XMLDB_INDEX_NOTUNIQUE, array('userid'));
        $dbman->drop_index($table, $index);

        $table = new xmldb_table('local_helpdesk_ticket_assign');
        $index = new xmldb_index('local_helpdesk_ticket_assign_u_ix');
        $index->set_attributes(XMLDB_INDEX_NOTUNIQUE, array('userid'));
        $dbman->add_index($table, $index);

        // on local_helpdesk_ticket_assign.ticketid
        $table = new xmldb_table('local_helpdesk_ticket_assign');
        ...
        $table = new xmldb_table('local_helpdesk_ticket_assign');
        $index = new xmldb_index('local_helpdesk_ticket_assign_t_ix');
        $index->set_attributes(XMLDB_INDEX_NOTUNIQUE, array('ticketid'));
        $dbman->add_index($table, $index);

        /// Define field type to be added to local_helpdesk_hd_user
        $table = new xmldb_table('local_helpdesk_hd_user');
        ...
        /// Define field token_last_issued to be added to local_helpdesk_watcher
        $table = new xmldb_table('local_helpdesk_watcher');
        ...
        $tickets = $DB->get_records_sql("
            ...
            LEFT JOIN {local_helpdesk_ticket_update} u ON u.ticketid = t.id
            WHERE t.status <> u.newticketstatus
            AND u.id = (
                SELECT u2.id FROM
                {local_helpdesk_ticket_update} u2
            ...
            FROM {local_helpdesk_ticket} t2
            WHERE NOT EXISTS (
                SELECT 1
                FROM {local_helpdesk_ticket_update} u3
        ...
        ");
        if ($tickets) {
            require_once("$CFG->dirroot/local/helpdesk/plugins/native/lib_native.php");
            foreach ($tickets as $t) {
                $sql = "
                    SELECT *
                    FROM {local_helpdesk_ticket_update} u
                ...
                if (!$DB->update_record('local_helpdesk_ticket_update', $update)) {
                    echo "Couldn't update ticket_update rec (ticket_update.id: $update->id) :(</ br>\n";
                }
            }
        }

        upgrade_local_savepoint(true, $version, 'helpdesk');
...
function xmldb_local_helpdesk_upgrade($oldversion = 0) {
    ...
    if ($result && $oldversion < 2010091400) {
        ...
        // populate all the statuses, which is normally done when the local is
    ...
    if ($result && $oldversion < 2013061702) {
        ...
        $dbman->rename_table($table, 'local_helpdesk');

        $table = new xmldb_table('helpdesk_ticket');
        $dbman->rename_table($table, 'local_helpdesk_ticket');

        $table = new xmldb_table('helpdesk_ticket_tag');
        $dbman->rename_table($table, 'local_helpdesk_ticket_tag');

        $table = new xmldb_table('helpdesk_ticket_update');
        $dbman->rename_table($table, 'local_helpdesk_ticket_update');

        $table = new xmldb_table('helpdesk_ticket_assignments');
        $dbman->rename_table($table, 'local_helpdesk_ticket_assign');

        $table = new xmldb_table('helpdesk_ticket_group');
        $dbman->rename_table($table, 'local_helpdesk_ticket_group');

        $table = new xmldb_table('helpdesk_status');
        $dbman->rename_table($table, 'local_helpdesk_status');

        $table = new xmldb_table('helpdesk_status_path');
        $dbman->rename_table($table, 'local_helpdesk_status_path');

        $table = new xmldb_table('helpdesk_rule');
        $dbman->rename_table($table, 'local_helpdesk_rule');

        $table = new xmldb_table('helpdesk_rule_email');
        $dbman->rename_table($table, 'local_helpdesk_rule_email');
    }
    ...
    if ($result) {
        $localhelpdeskhdusertable = new xmldb_table('local_helpdesk_hd_user');
        $hdusertableupgraded = false;
        if ($dbman->table_exists($localhelpdeskhdusertable)) {
    ...
    if ($oldversion < 2014041902) {
        ...
        // Define field detailformat to be added to local_helpdesk_ticket.
        $table = new xmldb_table('local_helpdesk_ticket');
        ...
        // Define field notesformat to be added to local_helpdesk_ticket_update.
        $table = new xmldb_table('local_helpdesk_ticket_update');
        ...
        // Define field valueformat to be added to local_helpdesk_ticket_tag.
        $table = new xmldb_table('local_helpdesk_ticket_tag');
        ...
        upgrade_local_savepoint(true, 2014041902, 'helpdesk');
    }
//tovi  2014042809
    if ($oldversion < 2014042810) {

        // Define field detailformat to be added to local_helpdesk_ticket.
        $table = new xmldb_table('local_helpdesk_ticket');
        $field = new xmldb_field('priority');
       //$field = new xmldb_field('detailformat', XMLDB_TYPE_INTEGER, '2', null, XMLDB_NOTNULL, null, '1', 'detail');
        $field->set_attributes(XMLDB_TYPE_INTEGER, '20', XMLDB_UNSIGNED, null, null, 2, null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        ...
        $priority->name        = 'low';
        $DB->insert_record('local_helpdesk_priority', $priority);
        $priority->name        = 'regular';
        $DB->insert_record('local_helpdesk_priority', $priority);
        $priority->name        = 'high';
        $DB->insert_record('local_helpdesk_priority', $priority);
        $priority->name        = 'immediately';
        $DB->insert_record('local_helpdesk_priority', $priority);
        $priority->name        = 'emergency';
        $DB->insert_record('local_helpdesk_priority', $priority);
    }

    if ($oldversion < 2014042809) {

        $table = new xmldb_table('local_helpdesk_institution');

        // Adding fields to table local_helpdesk_institution
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE);
        $table->add_field('institutionname', XMLDB_TYPE_CHAR, '100' , null, XMLDB_NOTNULL, null);
        $table->add_field('cooperative', XMLDB_TYPE_INTEGER, '1', null, XMLDB_NOTNULL, null);
        // Adding keys to table local_helpdesk_institution
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));
        $dbman->create_table($table);
    }
...
//tovi
    ...
    if ($result && $oldversion < 2014042802) {
        ...
        if (!$DB->count_records('role', array('shortname' => 'helpdeskmanager'))) {
            ...
            $HELPDESK_CAP_ASK = 'local/helpdesk:ask';
            $HELPDESK_CAP_ANSWER = 'local/helpdesk:answer';
            ...
            // Helpdesk savepoint reached.
            upgrade_local_savepoint(true, 2014042802, 'helpdesk');
