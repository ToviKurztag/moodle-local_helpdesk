...
if(empty($ticket)) {
    print_error('noticket', 'local_helpdesk');
}

$fc = $ticket->get_firstcontact();

if(!empty($fc)) {
    print_error('notavailabletograb', 'local_helpdesk');
}
...
if(!$ticket->add_assignment($USER->id)) {
    print_error('unabletoaddassignment', 'local_helpdesk');
}

$url = new moodle_url($CFG->wwwroot . '/local/helpdesk/view.php');
$url->param('id', $ticket->get_idstring());

redirect($url->out(), get_string('questiongrabbed', 'local_helpdesk') . ': ' . $ticket->get_summary());
