...
$update = $DB->get_record('local_helpdesk_ticket_update', array('id' => $id), 'id, hidden, ticketid');
$update->hidden = 1;
if(!$DB->update_record('local_helpdesk_ticket_update', $update)) {
    print_error('unabletohideupdate', 'local_helpdesk');
}

$url = new moodle_url("{$CFG->wwwroot}/local/helpdesk/view.php");
$url->param('id', $update->ticketid);
redirect($url->out(), get_string('updatewillnowbehidden', 'local_helpdesk'));
