...
class change_overview_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('header', 'frm', get_string('updateticketoverview', 'local_helpdesk'));
        $userparams = array('readonly' => 'readonly', 'disabled' => 'disabled');
        $mform->addElement('text', 'username', get_string('submittedby', 'local_helpdesk'), $userparams);
        ...
        $mform->addElement('select', 'status', get_string('updatestatus', 'local_helpdesk'),
                           $statuslist);
        ...
        $mform->addElement('header', 'notesheader', get_string('extrainformation', 'local_helpdesk'));
        $mform->addElement('editor', 'notes_editor', get_string('updatemessage', 'local_helpdesk'), null,
                $editoroptions);
        $mform->setType('notes_editor', PARAM_RAW);
        $mform->addRule('notes_editor', get_string('required'), 'required', null, 'client');

        $mform->closeHeaderBefore('submitbutton');
        $mform->addElement('submit', 'submitbutton', get_string('savequestion', 'local_helpdesk'));
