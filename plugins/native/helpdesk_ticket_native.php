...
class helpdesk_ticket_native extends helpdesk_ticket {
    ...
    function display_ticket($readonly=false) {
        ...
        $showfirstcontact = get_config(null, 'local_helpdesk_show_firstcontact');

        $user = helpdesk_get_hd_user($this->hd_userid);

        echo "<div class=\"ticketinfo\">";
        $overviewstr = get_string('ticketinfo', 'local_helpdesk');
        $overviewhelp = $OUTPUT->help_icon('ticketinfo', 'local_helpdesk');
        $editurl = new moodle_url("$CFG->wwwroot/local/helpdesk/edit.php");
        $editurl->param('id', $this->get_idstring());
        $editurl = $editurl->out();
        $editstr = get_string('editoverview', 'local_helpdesk');
        ...
        $row = array();
        $row[] = get_string('ticketid', 'local_helpdesk');
        ...
        if ($isanswerer and !$readonly) {
            $newuserurl = new moodle_url("$CFG->wwwroot/local/helpdesk/userlist.php");
            ...
            $str .= "<br /><small><a href=\"" . $newuserurl->out() . "\">" .
                   get_string('changeuser', 'local_helpdesk') . '</a></small>';
        }
        $row[] = $str;
        ...
        $row = array();
        $row[] = get_string('submittedby', 'local_helpdesk');
        $row[] = helpdesk_user_link($createdby);
        $table->data[] = $row;

        if ($this->firstcontact != null and $showfirstcontact != false) {
            $help = $OUTPUT->help_icon('firstcontact', 'local_helpdesk');

            $row = array();
            $row[] = get_string('firstcontactuser', 'local_helpdesk') . $help;
            $row[] = helpdesk_user_link($this->firstcontact);
            $table->data[] = $row;
        }

        $row = array();
        $row[] = get_string('timecreated', 'local_helpdesk');
        $row[] = helpdesk_get_date_string($this->get_timecreated());
        $table->data[] = $row;

        $row = array();
        $row[] = get_string('timemodified', 'local_helpdesk');
        $row[] = helpdesk_get_date_string($this->get_timemodified());
        $table->data[] = $row;

        $row = array();
        $row[] = get_string('priority', 'local_helpdesk');
        $row[] = $this->get_priority_string($this->priority);
        $table->data[] = $row;

        $row = array();
        $row[] = get_string('institution', 'local_helpdesk');
        $row[] = $hd->get_user_institution($this->hd_userid);
        $table->data[] = $row;

        $row = array();
        $row[] = get_string('status', 'local_helpdesk');
        $status = $this->get_status();
        if ($status->core == true and empty($status->displayname)) {
            $row[] = get_string($status->name, 'local_helpdesk');
        } else {
        ...
        $row = array();
        $row[] = get_string('summary', 'local_helpdesk');
        $row[] = $this->get_summary();
        $table->data[] = $row;

        $row = array();
        $row[] = get_string('detail', 'local_helpdesk');
        $row[] = file_rewrite_pluginfile_urls($this->get_detail(), 'pluginfile.php',
            context_system::instance()->id, 'local_helpdesk', 'ticketdetail', $this->get_idstring());
        $table->data[] = $row;
        ...
        if (empty($this->firstcontact) and helpdesk_is_capable(HELPDESK_CAP_ANSWER)) {
            $row = array();
            $row[] = get_string('answerertools', 'local_helpdesk');
            $url = "{$CFG->wwwroot}/local/helpdesk/plugins/native/action/grab.php";
            $grab_help = $OUTPUT->help_icon('grabquestion', 'local_helpdesk');
            $row[] = "<form action=\"{$url}\" method=\"get\">"
                . '<input type="hidden" name="id" value="'
                . $this->get_idstring() . '" />'
                . '<input type="submit" value="'
                . get_string('grabquestion', 'local_helpdesk')
                . "\" />{$grab_help}</form>";
            $table->data[] = $row;
        }
       ...
       // echo '<br />';
        ...
        $assignedstr = get_string('assignedusers', 'local_helpdesk');
        $assignedhelp = $OUTPUT->help_icon('assignedusers', 'local_helpdesk');
        $thead = $assignedstr . $assignedhelp;

        // If the user is a answerer, he can assign people to the ticket.
        if ($isanswerer and !$readonly) {
            $url = new moodle_url("$CFG->wwwroot/local/helpdesk/assign.php");
            $url->param('tid', $this->get_idstring());
            $url = $url->out();
            $string = get_string("assignuser", 'local_helpdesk');
        ...
        if ($assigned === false) {
            $table->data = array(array(get_string('noneassigned', 'local_helpdesk')));
        } else {
            $table->data = array();

            foreach ($assigned as $user) {
                $removeurl = new moodle_url("$CFG->wwwroot/local/helpdesk/assign.php");
        ...
        if ($isanswerer) {
            $watcherstr = get_string('watchingusers', 'local_helpdesk');
            $whead = $watcherstr . $OUTPUT->help_icon('watchingusers', 'local_helpdesk');
            if (!$readonly) {
                $addurl = new moodle_url("$CFG->wwwroot/local/helpdesk/userlist.php");
                $addurl->param('function', HELPDESK_USERLIST_NEW_WATCHER);
                $addurl->param('tid', $this->get_idstring());
                $whead .= '<br /><a href="' . $addurl->out() . '">' . get_string('assignwatchers', 'local_helpdesk') . '</a>';
            }
            ...
            if (!$watchers = $this->get_watchers()) {
                $table->data = array(array(get_string('nowatchers', 'local_helpdesk')));
            } else {
                $removeurl = new moodle_url("$CFG->wwwroot/local/helpdesk/manage_watchers.php");
                $removeurl->param('tid', $this->get_idstring());
                $removeurl->param('remove', 'true');
                $table->data = array();
        ...
        $table->size = array('30%');
        $tagstr = get_string('extradetailtags', 'local_helpdesk');
        $taghelp = $OUTPUT->help_icon('extradetailtags', 'local_helpdesk');
        ...
        if ($isanswerer and !$readonly) {
            ...
            $url->param('tid', $this->get_idstring());
            $url = $url->out();
            $addtagstr = get_string('addtag', 'local_helpdesk');
            ...
            foreach ($tags as $tag) {
                ...
                $table->data[] = array(
                    ...
                    file_rewrite_pluginfile_urls($tag->value, 'pluginfile.php',
                        context_system::instance()->id, 'local_helpdesk', 'tickettag', $tag->id)
                );
            }
        } else {
            $table->data = array(array(get_string('notags', 'local_helpdesk')));
        }
        ...
        // Updates start here.
        $updatestr = get_string('updates', 'local_helpdesk');
        $updatehelp = $OUTPUT->help_icon('updates', 'local_helpdesk');
        ...
        $url = $url->out();
        $translated = get_string('updateticket', 'local_helpdesk');

        $thead = "$updatestr $updatehelp";
        if (!$readonly) {
            $thead .= "<a href=\"$url\">$translated</a>";
        }
        $table->head = array($thead);
        ...
        if (is_array($udata) or is_object($udata)) {
            ...
            foreach ($udata as $update) {
                ...
                if ($update->type !== false and
                    ...
                    $row = array();
                    $str = get_string($update->type, 'local_helpdesk');
                    $row[] = $str;
                    if ($hd->is_update_hidden($update)) {
                        $url = new moodle_url("{$CFG->wwwroot}/local/helpdesk/plugins/native/action/showupdate.php");
                        $str = get_string('showupdate', 'local_helpdesk');
                    } else {
                        $url = new moodle_url("{$CFG->wwwroot}/local/helpdesk/plugins/native/action/hideupdate.php");
                        $str = get_string('hideupdate', 'local_helpdesk');
                    }
                    $url = $url->out();
                    if($isanswerer) {
                    ...
                    if ($hd->is_update_hidden($update)) {
                        $row[] = get_string('thisupdateishidden', 'local_helpdesk') . '<br />'
                            . $hideshowbutton;
                    } else {
                        $row[] = $hideshowbutton;
                    }
                ...
                if (!$user) {
                    print_error(getstring('unabletopulluser', 'local_helpdesk'));
                }
 //Toviii
                // Status
                //     $row = array();
                //     $row[] = get_string('status', 'local_helpdesk');
                //     //$row[] = get_string($update->status, 'local_helpdesk');
                //    //Tovii
                //    $row[] = 'status';
                ...
                // $row = array();
                // $row[] = get_string('user', 'local_helpdesk');
                // $row[] = helpdesk_user_link($user);
                // $update_table->data[] = $row;
               
                // if ($update->newticketstatus != null) {
                //     $row = array();
                //     $tstat = $DB->get_record('local_helpdesk_status', array('id' => $update->newticketstatus));
                //     $row[] = get_string('newquestionstatus', 'local_helpdesk');
                ...
                // // Update Note.
                // $row = array();
                // $row[] = get_string('note', 'local_helpdesk');
                // $row[] = file_rewrite_pluginfile_urls($update->notes, 'pluginfile.php',
                //     context_system::instance()->id, 'local_helpdesk', 'ticketnotes', $update->id);
                // $update_table->data[] = $row;
                // $table->data[] = array(html_writer::table($update_table));
                // //                echo '<br />';

                $rowuser = array();
                $rowuser[] = get_string('user', 'local_helpdesk');
                $rowuser[] = helpdesk_user_link($user);

               // if ($update->newticketstatus != null) {
                    $rowstatus = array();
                    $tstat = $DB->get_record('local_helpdesk_status', array('id' => $update->newticketstatus));
                    $rowstatus[] = get_string('status', 'local_helpdesk');
                    $rowstatus[] = $this->get_status_string($tstat);
               // }
                ...
                // Time Created date.
                $rowdate[] = get_string('timecreated', 'local_helpdesk');
                $rowdate[] = $creation_date;

                // Update Note.
                $rownote = array();
                $rownote[] = get_string('note', 'local_helpdesk');
                $rownote[] = file_rewrite_pluginfile_urls($update->notes, 'pluginfile.php',
                    context_system::instance()->id, 'local_helpdesk', 'ticketnotes', $update->id);

                $head = array();
                $head[] = $rowuser[0]. ":<br/>". $rowuser[1];
                $head[] = $rowstatus[0]. ":<br/>" . $rowstatus[1];
                $head[] = $rowdate[0]. ":<br/>" . $rowdate[1];
                $head[] = $hideshowbutton;
                // $row1 =  $rowuser[0]." : ". $rowuser[1];
                // $row2 =  $rowstatus[0]. " : " . $rowstatus[1] ."  ". $rowdate[0]. "  : " . $rowdate[1] ."  ". $hideshowbutton;
               // print_r($rowuser[0]);die;
                // $head = array();
                // $head[] = $row1;
                // $head[] = $row2;

                $update_table->head = $head;
              //  print_r("<td colspan='3'>hhhhhhhhhhh</td>");die;

                $update_table->data[] = $rownote;
               // $update_table->data[] = $row;
//print_r($update_table);die;
                $table->data[] = array(html_writer::table($update_table));
                               echo '<br />';
        ...
        if ($updateprinted === false) {
            $row = array();
            $row[] = get_string('noupdatestoview', 'local_helpdesk');
            $update_table->data[] = $row;
    ...
    function set_status($status) {
        ...
        if (is_numeric($status)) {
            $status = $DB->get_record('local_helpdesk_status', array('id' => $status));
        }
        if (!is_object($status)) {
    ...
    /**
     * Get method that returns the priority.
     *
     * @return string
     */
    function get_priority_string($priorityid) {

        global $DB;
        $sql = ' select distinct p.*
        from mdl_local_helpdesk_ticket as t
        join mdl_local_helpdesk_priority as p
        on t.priority = p.id
        where t.priority = ?';

        $priority = $DB->get_record_sql($sql, [$priorityid], $strictness = IGNORE_MISSING);
        return get_string($priority->name, 'local_helpdesk');
        //return $priority->name;
    }

    /**
     * Get method that returns the user firstname and lastname.
     *
     * @return string
     */
    function get_user_name($hduserid) {
        global $DB;
     ...
     * @return string
     */
    function get_assigned_string($assignedusers) {
        $str = '';
        if (is_array($assignedusers) || is_object($assignedusers)) {
     ...
     * @return array
     */
    function get_user_updates($includehidden=false) {
        if ($includehidden == true) {
            return $this->updates;
        }
        $updates = array();
        if (!empty($this->updates)) {
            foreach($this->updates as $update) {
                ...
                if ($update->type != 'update_type_user' || empty($update->notes)){
                    continue;
                }
                $updates[] = $update;
            }
    ...
    function get_assign_updates($includehidden=false) {
        if ($includehidden == true) {
            return $this->updates;
        }
        $updates = array();
        if (!empty($this->updates)) {
            foreach($this->updates as $update) {
                ...
                if ($update->type != 'update_type_detailed' || empty($update->notes)){
                    continue;
                }
                $updates[] = $update;
            }
    ...
    function get_responded_ticket($ticket, $hduseridsuporter) {
        global $DB;
       // print_r($hduserid);die;
        $sql = 'select h.*
                from mdl_local_helpdesk_ticket_update as h
                where h.timemodified >= ? AND h.hd_userid != ?
                AND h.ticketid = ?
                order by h.timemodified DESC 
                ';
        $params = array('timemodified' => $ticket->get_timemodified(), 'hd_userid' => $hduseridsuporter, 'ticketid' => $ticket->id);
       // $DB->set_debug(1);
    ...
    function get_status_string($status=null) {
        ...
        if ($status->core == true and empty($status->displayname)) {
            return get_string($status->name, 'local_helpdesk');
        }
...
// <Tovi
     ...
     * @return mixed
     */
    // function get_priority_string($priority=null) {
    //     // Matt thinks this is evil. Now that we're moving statuses to the
    //     // database, we need this to do some pre-processing of statuses.
    //     if ($priority != null and !is_object($priority)) {
    //         print_error('non-object ('.gettype($priority).') passed to get_status_string()');
    //     }
    //     return get_string($priority->name, 'local_helpdesk');
    // }
// Tovi>
    ...
    function add_assignment($userid) {
        ...
        if (!$DB->insert_record('local_helpdesk_ticket_assign', $assign)) {
            return false;
        }
        ...
        $update->notes_editor['text'] = fullname_nowarnings($user) . ' '
            . get_string('wasassigned', 'local_helpdesk');
        $update->notes_editor['format'] = FORMAT_HTML;
        if(!$this->add_update($update)) {
            echo $OUTPUT->notification(get_string('cantaddupdate', 'local_helpdesk'));
        }

        if (!empty($CFG->local_helpdesk_assigned_auto_watch)) {
    ...
    function remove_assignment($userid) {
        ...
        $result = $DB->delete_records('local_helpdesk_ticket_assign', array('userid' => $userid, 'ticketid' => $this->id));
        if ($result) {
            $this->store();
            $user = helpdesk_get_user($userid);
            $update = (object) array(
            ...
            $update->notes_editor['text'] = fullname_nowarnings($user) . ' ' .
                get_string('wasunassigned', 'local_helpdesk');
            $update->notes_editor['format'] = FORMAT_HTML;
        ...
        if (!empty($CFG->local_helpdesk_assigned_auto_watch)) {
            if (!$this->remove_watcher($user->hd_userid)) {
                print_error('cannotremovewatcher', 'local_helpdesk');
            }
        }
        return $result;
    }
    ...
    function get_assigned() {
        ...
        $records = $DB->get_records('local_helpdesk_ticket_assign', array('ticketid' => $this->id));
    ...
    function add_watcher($hd_userid) {
        ...
        if (!$DB->insert_record('local_helpdesk_watcher', $watcher)) { return false; }

        // update
        $update = (object) array(
            'notes_editor' => array('text' => fullname_nowarnings($user) . get_string('startwatching', 'local_helpdesk'),
                'format' => FORMAT_HTML),
    ...
    function remove_watcher($hd_userid) {
        ...
        $result = $DB->delete_records('local_helpdesk_watcher', array('hd_userid' => $hd_userid, 'ticketid' => $this->id));
        if ($result) {
            $this->store();
            $user = helpdesk_get_hd_user($hd_userid);
            $update = (object) array(
                'notes_editor' => array('text' => fullname_nowarnings($user) . get_string('notwatching', 'local_helpdesk'),
                    'format' => FORMAT_HTML),
    ...
    function get_watchers() {
        ...
        $records = $DB->get_records('local_helpdesk_watcher', array('ticketid' => $this->id));
    ...
    function fetch($permissionhalt=true) {
        ...
        $ticket = $DB->get_record('local_helpdesk_ticket', array('id' => $this->id));
        if (!$ticket) {
            return false;
        }
        ...
        $watchers = $DB->get_records('local_helpdesk_watcher', array('ticketid' => $this->id));
      //  print_r($watchers);die;
        ...
        // <Toviii put for instution tickets.
        //print_r($iswatcher);die;
        ...
        $updates        = $DB->get_records('local_helpdesk_ticket_update', array('ticketid' => $this->id), 'timecreated DESC');
        $tags           = $DB->get_records('local_helpdesk_ticket_tag', array('ticketid' => $this->id), 'name ASC');
        $this->status   = $DB->get_record('local_helpdesk_status', array('id' => $this->status));
    ...
    function store() {
        ...
        if (empty($this->status)) {
            $this->status               = $DB->get_record('local_helpdesk_status', array('ticketdefault' => 1));
        }
        ...
        if (!empty($dataobject->id)) {
            if ($result = $DB->update_record('local_helpdesk_ticket', $dataobject)) {
                //print_r($dataobject);die;
                $this->fetch();
            //     $hd = helpdesk::get_helpdesk();
            //    $rval = $hd->email_assign($this, true);
               //hereeeeeeeeeeeeeee
            //   print_r($this);die;
                return true;
            }
        ...
        $result = $DB->insert_record('local_helpdesk_ticket', $dataobject);
        if ($result) {
           // print_r($dataobject);die;
            $this->id = $result;
        } else {
            print_error('error_insertquestion', 'local_helpdesk');
        }

        # add the submitter as a watcher
        $this->add_watcher($this->hd_userid);

        if($this->createdby === $this->hd_userid and get_config(null, 'local_helpdesk_includeagent') == true) {
            $tag = new stdClass;
            $tag->name = get_string('useragent', 'local_helpdesk');
            $tag->value = $_SERVER['HTTP_USER_AGENT'];
            $tag->ticketid = $this->get_idstring();
            $this->add_tag($tag);
            $tag = new stdClass;
            $tag->ticketid = $this->get_idstring();
            $tag->name = get_string('useroperatingsystem', 'local_helpdesk');
    ...
    function add_update($update) {
      //  print_r($update);die;
       ...
       // $dat->notes         =  $update->notes;
        ...
        //Toviii
        $dat->status        = $status->id;
        //$dat->status        = $update->status;
        $dat->priority      = $update->priority;
        ...
        if ( $updateid = $DB->insert_record('local_helpdesk_ticket_update', $dat) ) {
            $usefirstcontact = get_config(null, 'local_helpdesk_firstcontact');
            $isanswerer = helpdesk_is_capable(HELPDESK_CAP_ANSWER);
            ...
            $dat = file_postupdate_standard_editor($dat, 'notes', $editoroptions, $context,
                'local_helpdesk', 'ticketnotes', $updateid);

            $updateid = $DB->update_record('local_helpdesk_ticket_update', $dat);
            //update_type_detailed
            if ($dat->type == HELPDESK_UPDATE_TYPE_USER) {
                $rval = $hd->email_update($this);
            }
            //|| strpos($dat->notes, 'was unassigned from the question') !== false
            if (strpos($dat->notes, 'was assigned to this question') !== false)  {
                //print_r($dat);die;
                $rval = $hd->email_assign($this);
            }
            return true;
        }
    ...
    private function process_update($update) {
      ...
      //  print_r($update);die;
        ...
        if (is_numeric($update->status)) {
            $this->status = $DB->get_record('local_helpdesk_status', array('id' => $update->status));
    ...
    function update_tag($tag) {
        ...
        $tag = file_postupdate_standard_editor($tag, 'value', $editoroptions, $context,
            'local_helpdesk', 'tickettag', $tag->id);

        if ($DB->update_record('local_helpdesk_ticket_tag', $tag)) {
            $this->store();
            $this->fetch();
            return true;
        }
    ...
    function add_tag($tag) {
        ...
        $tag->value = '';
        if (!$tag->id = $DB->insert_record('local_helpdesk_ticket_tag', $tag)) {
            return false;
        }
        ...
        $dat = new stdClass;
        $dat->ticketid  = $this->id;
        $dat->notes_editor = array('text' => get_string('tagaddedwithnameof', 'local_helpdesk') . $tag->name,
            'format' => FORMAT_HTML);
    ...
    function remove_tag($id) {
        ...
        $tag = $DB->get_record('local_helpdesk_ticket_tag', array('id' => $id));

        $result = $DB->delete_records('local_helpdesk_ticket_tag', array('id' => $id));
        if (!$result) {
            return false;
        }
        // Lets make an update!
        ...
        $dat = new stdClass;
        $dat->ticketid      = $this->id;
        $dat->notes_editor  = array('text' => get_string('tagremovewithnameof', 'local_helpdesk') . $tag->name,
            'format' => FORMAT_HTML);
    ...
    function store_edit($noteseditor=null, $newstatus=null) {
        ...
        if (!$this->add_update($update)) {
            echo $OUTPUT->notification(get_string('unabletoaddeditupdate', 'local_helpdesk'));
        }
        return true;
    }
    ...
    public function set_firstcontact($user) {
        ...
        if(empty($user)) {
            print_error('invalidtype', 'local_helpdesk');
        }
        if(empty($this->firstcontact)) {
