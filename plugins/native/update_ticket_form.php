...
class update_ticket_form extends moodleform {
    function definition() {
        ...
        $mform->addElement('header', 'frm', get_string('updateticket', 'local_helpdesk'));

        $mform->addElement('editor', 'notes_editor', get_string('notes', 'local_helpdesk'), null, $editoroptions);
        ...
        $sql = "SELECT s.*
                FROM {local_helpdesk_status} s
                    JOIN {local_helpdesk_status_path} sp ON sp.tostatusid=s.id
        ...
        $mform->addElement('select', 'status', get_string('updatestatus', 'local_helpdesk'), $statuslist);
        ...
        if (helpdesk_is_capable(HELPDESK_CAP_ANSWER)) {
            $mform->addElement('checkbox', 'hidden', get_string('hideupdate', 'local_helpdesk'));
        }

        $mform->addElement('submit', 'submitbutton', get_string('updateticket', 'local_helpdesk'));
    }
