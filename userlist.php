...
$thisurl = new moodle_url("$CFG->wwwroot/local/helpdesk/userlist.php");
...
if (!empty($ticketid)) {
    $ticketreturn = new moodle_url("$CFG->wwwroot/local/helpdesk/view.php");
...
$title = get_string('helpdeskselectuser', 'local_helpdesk');
$selecttext = get_string('selectuser', 'local_helpdesk');
if (!isset($userset)) {
...
case HELPDESK_USERLIST_NEW_SUBMITTER:
    ...
    $returnurl = new moodle_url("$CFG->wwwroot/local/helpdesk/edit.php");
    $returnurl->param('id', $ticketid);
    $paramname = 'newuser';

    $nav[] = array (
        'name' => get_string('updateticketoverview', 'local_helpdesk'),
        'link' => $returnurl->out()
    );
    $nav[] = array (
        'name' => get_string('changesubmitter', 'local_helpdesk'),
    );
    $title = get_string('helpdeskchangeuser', 'local_helpdesk');
    break;
case HELPDESK_USERLIST_NEW_WATCHER:
    $thisurl->param('tid', $ticketid);
    $returnurl = new moodle_url("$CFG->wwwroot/local/helpdesk/manage_watchers.php");
    $returnurl->param('tid', $ticketid);
    $paramname = 'hd_userid';

    $nav[] = array(
        'name' => get_string('addwatcher', 'local_helpdesk'),
    );
    $title = get_string('helpdeskselectwatcher', 'local_helpdesk');
    break;
case HELPDESK_USERLIST_MANAGE_EXTERNAL:
    ...
    $returnurl = new moodle_url("$CFG->wwwroot/local/helpdesk/hduser.php");
    $returnurl->param('returnurl', $thisurl->out());
    $paramname = 'id';
    $nav[] = array(
        'name' => get_string('manageexternal', 'local_helpdesk'),
    );
    $title = get_string('helpdeskmanageexternal', 'local_helpdesk');
    $selecttext = get_string('edituser', 'local_helpdesk');
    break;
case HELPDESK_USERLIST_SUBMIT_AS:
    $returnurl = new moodle_url("$CFG->wwwroot/local/helpdesk/new.php");
    $paramname = 'hd_userid';
    $nav[] = array(
        'name' => get_string('helpdesknewticket', 'local_helpdesk'),
    );
    $title = get_string('helpdeskselectticketuser', 'local_helpdesk');
    break;
...
$nav[] = array (
    'name' => get_string('selectuser', 'local_helpdesk'),
);
...
// new user
if ($CFG->local_helpdesk_allow_external_users and !$hd_userid) {
    $user_form = new helpdesk_user_form(true, "$CFG->wwwroot/local/helpdesk/hduser.php");
...
if ($function == HELPDESK_USERLIST_MANAGE_EXTERNAL and $hd_userid) {
    ...
    $sql_users = "
        FROM {user} AS u
        LEFT JOIN {local_helpdesk_hd_user} AS hu2 ON hu2.userid = u.id
        WHERE u.deleted = 0
    ";
    $sql_hdusers = "
...
foreach($users as $user) {
    ...
    $table->data[] = array(
        ...
        $user->email,
        $user->phone,
        isset($user->userid) ? get_string('internal_user', 'local_helpdesk') : $user->type,
    );
}
