...
class helpdesk_user_form extends moodleform {
    ...
    function definition() {
        ...
        $mform->addElement('header', 'title', get_string($this->new_user ? 'new_user' : 'editexternal', 'local_helpdesk'));
        $mform->addElement('text', 'name', get_string('fullname'), 'size="40"');
        $mform->addRule('name', null, 'required');
        $mform->setType('name', PARAM_TEXT);
        $mform->addElement('text', 'email', get_string('email', 'local_helpdesk'), 'size="40"');
        ...
        $type_options = array_map(function ($option) {
                return trim($option);
            }, explode(',', $CFG->local_helpdesk_user_types));
        $type_options = array_combine($type_options, $type_options);
        $mform->addElement('select', 'type', get_string('usertype', 'local_helpdesk'), $type_options);
