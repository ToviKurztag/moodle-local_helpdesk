...
abstract class helpdesk {
    ...
    /**
     * Every helpdesk has access to the moodle cron for this local. This method 
     * gets called every time cron hits the local.
     *
     * @return true
     */
    ...
    public static function page_init($title, $nav=array(), $url=null) {
        ...
        // We also have this neat navigation inside the navigation local which
        ...
        // Desk context.
        $hd_nav = $PAGE->navigation->add(get_string('helpdesk', 'local_helpdesk'));
    ...
    public final static function get_helpdesk() {
        ...
        $initpath = "{$CFG->dirroot}/local/helpdesk/plugins/$plugin/init.php";

        if (!file_exists($initpath)){
            print_error('missingpluginfile', 'helpdesk_local');
        }
    ...
    function new_search_obj() {
        return (object)array(
            ...
            'watcher' => 0,
            'institution' => 0,
            'timecreatedfrom' => strtotime("-1 year", time()),
            'timecreatedto' => time()
        );
    }
}
