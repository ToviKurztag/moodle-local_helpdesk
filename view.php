...
if (strlen($token)) {
    ...
    $readonly = empty($CFG->local_helpdesk_external_updates);
    $nav = array();
} else {
    ...
    $nav = array(
        ...
        array(
            'name' => get_string('ticketviewer', 'local_helpdesk')
        )
    );
}
$title = get_string('helpdesk', 'local_helpdesk');
helpdesk::page_init($title, $nav);
echo $OUTPUT->header();

echo $OUTPUT->heading(get_string('ticketviewer', 'local_helpdesk'));
...
if (!$ticket) {
    print_error('ticketiddoesnotexist', 'local_helpdesk');
}
